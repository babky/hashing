...
Notice one way of extraction of a limit function when we know $\Expect{\lpsl}$. From Markov inequality it follows that $\Prob{\lpsl \ge k \Expect{\lpsl}} \leq \frac{1}{k}$. Better limit functions may be found if we have an upper bound on probability distribution function of the random variable $\lpsl$.
...
The two choices paradigm comes from the area of balls and bins systems. For separate chaining with a single perfectly random function it is known that $\Expect{\lpsl} = \bigo\left({\log n}/{\log \log n}\right)$. A proof of this bound may be found in \cite{DBLP:books/sp/Mehlhorn84}. The same bound also holds with a single function chosen from an almost uniform system. On the other hand with a limited independence this problem becomes harder and general bounds for universal hashing are not available.
...
In the following definition for an $\vec{x} \in U^k$ and $f \in H$ the notation $f(\vec{x})$ means a vector from $V^k$ and $f(\vec{x}) = (f(x_1), \dots, f(x_k))$.
...
\end{split}
\]
The positive fact is that this probability tends to one as $n$ increases. A more unpleasant thing is that choosing a new function means regenerating the system and takes $\bigo(n)$ time.
%^TODO: tends?
...
In case of the two choices paradigm the limit function is $\bigo(\log \log n)$ and with a~single hash function $\bigo(\log n / \log \log n)$ and these bounds are tight.
