...
The expected complexity of a sequence of operations $\Expect{T}$ then equals
\[
\Expect{T} = \sum_{i=1}^{k} \Expect{t_i} = \sum_{i = 1}^{k} \left(\Expect{a_i} - \Expect{p_i} + \Expect{p_{i - 1}}\right) = \Expect A - \Expect{p_k} + \Expect{p_0}.
\]
