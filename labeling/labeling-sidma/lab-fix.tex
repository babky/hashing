...
To prove the lower bound we construct an adversary generating input for a given online labeling algorithm so that the algorithm is forced to make many relabelings.
A substantial property of the adversary is that the run of any online labeling algorithm against the constructed adversary naturally gives a strategy of a game we call \emph{Prefix Bucketing}, which is defined further in the text.
...
Finally we provide a lower bound on the cost of each prefix bucketing strategy which immediately gives a tight lower bound for \emph{Online Labeling}.
In the remainder of the introduction we sketch the parts in more detail, and relate our proof to the one in \cite{DSZ04}.
...
Before the 8-th insertion, the hierarchy consists of just the single segment $\natInt{1}{m}$ consisting of the equally spaced 7 items.
...
\emph{Prefix bucketing of $n$ items into $k$ buckets} (numbered 1 to $k$) is a one player game consisting of $n$ steps.
...
in the critical segment. In the case when the critical segment has no successor (sub)segment, then the amount of items 
in the critical segment is less than a constant defined later and the cost of the online labeling step is exactly one.


\paragraph{Improved Analysis of Bucketing}
...
and satisfies  $\weight^{t-1}(R')=\lfloor \weight^{t-1}(S')/2 \rfloor$.
...
$\weight^{t-1}(T) = \lfloor \weight^{t-1}(S')/2 \rfloor$, the proposition follows.
\end{proof} 

%%%END MIKE 5-13-15

Suppose $S$ is a segment with $\weight^{t-1}(S)=\ell \geq 2$.  Let $x_1< \ldots < x_\ell$ denote the elements
of $\pop^{t-1}(S)$.  We define $\midp^{t-1}(S)= \lceil (x_{\lceil (\ell-1)/2 \rceil} + x_{\lceil (\ell+1)/2 \rceil} )/ 2 \rceil$. Thus $\midp^{t-1}(S)$ is obtained by averaging the two middle items whose labels belong to  $S$. 
...
the smallest segment that contains $f^t(Rel^t)\cup f^{t-1}(Rel^t \setminus \{y^t\})$. (As usual, $f(A) = \{f(x) : x \in A\}.$)
%%%MIKE BEGIN 5/3/15
...
We present the \emph{Adversary} which generates the input. Recall that the adversary keeps 
the hierarchy of nested \emph{segments} having, imprecisely said, increasing \emph{density}. 
...
from the \emph{critical segment}.
...
\smallskip\noindent Set $p^0=0$.
\fi
\item  \emph{The first 7 steps:} For $t$ from 1 to 7, set $y^t= 1+ (t-1)2^{n-7}$.
...
\paragraph{Algorithm $A_1$} 
While  no two adjacent cells are occupied, insert the next item $y$ into the middle cell of the
...
For $m \geq 3$, and $2 \leq k < r(m)/2$, $A_k$ consists of two main {\em phases} using the algorithm $A_{k-1}$ on the whole array and algorithms $A_{k}$ on smaller arrays.
\begin{itemize}
	\item {\bf Phase 1.} Insert $b_{k-1}(m)$ items using algorithm $A_{k-1}$.
	\item {\bf Redistribution step.} Redistribute the items evenly so that each of the $w=1+b_{k-1}(m)$ segments has size at least $\lceil m/w \rceil$. 
	\item {\bf Phase 2.} Items placed during phase 1 do not move. We run $w$ separate instances of $A_k$, one for each  phase 1 segment. Each arriving item is assigned to the phase 1 segment that is compatible with it,
		and is inserted into that segment using $A_k$. 
\end{itemize}
The total cost per item inserted in phase 1 is at most $k$.
We call the segments defined by the items after the redistribution step the {\em phase 1 segments}.   
Each phase 1 segment has length at least $\lceil m/w \rceil $ and so we can handle $b_k(m/w)$ insertions 
