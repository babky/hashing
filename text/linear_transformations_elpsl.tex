...
\end{theorem}
\begin{proof}
The target space, representation of the hash table, let be denoted by $B = \vecspace{t}$. As in the previous proof we will factor a linear transformation $T \in H$, $T: U \rightarrow B$. We use the vector space $A = \vecspace{l}$ for $l \geq \log n$ and two functions $T_0: D \rightarrow A$ and $T_1: A \rightarrow B$ which is surjective. Both linear functions $T_0$ and $T_2$ are selected uniformly. This implies the uniform choice of the transformation $T = T_1 \circ T_0$ because of model \ref{remark-model-uniform-linear-map-selection}.

The idea presented here is to estimate the probability $\Prob{\lpsl > t}$ by using the other somehow unnatural event $E_2$. The probabilities are then easier to find. Probability of event $E_1 \equiv \lpsl > t$ can be simply determined by discovering the probability $\Prob{E_2 | E_1}$. When the $E_1$ is true with a high probability the event $E_2$ also appears.
