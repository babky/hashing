...
Let $T: \vecspace{u} \rightarrow \vecspace{b}$ be a random uniformly chosen linear transformation, $\epsilon \in (0, 1)$ and $\alpha \in \left[0.5, 1\right]$. If $S$ is a subset of the vector space $\vecspace{u}$ such that $|S| = \alpha 2 ^ b$, then \[ \Expect{\lpsl} \leq 4 \alpha c_\epsilon(4 + I_\epsilon) b \log b \text{.} \] The value $I_\epsilon$ is defined in the proof of Theorem \ref{theorem-n-logn-to-n} by Equality \ref{equality-i-e}.
\end{theorem}
\begin{proof}
