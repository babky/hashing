...
\begin{theorem}
Let $H$ and $I$ be $k$-strongly universal systems both over the universe $U$ and table of size $m$. Then the system of pairs of functions 
\begin{displaymath}
J = \lbrace f_{(h, i)}:U \times U \rightarrow \lbrace 0, \dots, m - 1 \rbrace \times \lbrace 0, \dots, m - 1 \rbrace | h \in H, i \in I \rbrace
\end{displaymath} 
is also $k$-strongly universal. The result of a composite function given by a pair $(h, i)$ is computed as $f_{(h, i)}(x) = (h(x), i(x))$.
\end{theorem}
\begin{proof}
Let $x_1, \dots, x_k$ be the elements that are supposed to be mapped onto selected buckets $(y_1, z_1), \dots, (y_k, z_k)$. The number of functions mapping both $h(x_1) = y_1, \dots, h(x_k) = y_k$ and $h(x_1) = z_1, \dots, h(x_k) = y_k$ is $\frac{|H|}{m^k}$ or $\frac{|I|}{m^k}$ respectively. So the number of all functions mapping $h(x_1) = (y_1, z_1), \dots, h(x_k) = (y_k, z_k)$ is equal to $\frac{|H||I|}{m^{2k}}$. Although the exponent is $2k$ because the table size is $m^2$ the constructed system is $k$-strongly universal only.
\end{proof}

Previous result limits the power of combining two system into a larger table. We obtain a smaller probability of collision but we pay an expensive price of the larger table size. The same effect on the collision probability can be simply achieved by squaring the table size. 

We could also combine the two functions and use a suitable operation to merge the given results into a smaller table as it is stated in the next theorem.
\begin{theorem}
...
\begin{corollary}
Combinations of functions from $k$-strongly universal classes with the operation:
\begin{itemize}
\item $xor$ where the table size is a power of 2
\item $+$, $-$ when the table size is a prime
\end{itemize}
