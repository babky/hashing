...
System of linear functions was among the first systems of universal hash functions. They were introduced by Carter and Wegman in \cite{DBLP:journals/jcss/CarterW79} where they showed basic properties of universal hashing. Above all they mentioned the expected constant time of the find operation.
...
By using this idea to construct a fast hash table we are trapped in a cycle since we already needed one. But $\omega$-universal class can not only be used to construct a fast solution to the dictionary problem but to solve other problems like set equality. %TODO: \cite{nieco}
...
From the previous theorem we have that probability of event $h(x_1) = a_1, \dots, h(x_k) = a_k$ and $i(x_1) = b_1, \dots, i(x_k) = b_k$ is less than $\frac{1}{m^{2k}}$. Now we see that $P(f(x_i) = y_i, 1 \le i \le k) = \frac{m^k}{m^{2k}} = \frac{1}{m^k}$. Now we see the system $F$ is strongly $k$-universal.
\end{proof}
