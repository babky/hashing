...
\end{theorem}
\begin{proof}
First we will construct the sequence of sets $U_0$, $U_1$, $\dots$. The set $U_0$ is the same as $U$. The set $U_i$ denotes the greatest subset of $U_{i - 1}$ that collides under the function $h_i \in H$. From the Dirichlet's principle the size of every set $U_i$ may be estimated as $|U_i| \geq \frac{|U_{i - 1}|}{m}$ . By using a bounded induction one can obtain $|U_i| \geq \frac{|U|}{m^k}$.

Now let the index $i$ denote the last set $U_i$ such that $|U_i| \geq k$. The functions $h_1, h_2,\dots, h_i$ map at least $k$ elements of the set $U_i$ to a single image. The probability of this event is less or equal to $\frac{1}{m ^ {k - 1}}$. From this argument we now know that $i \leq \frac{|H|}{m ^ {k - 1}}$. Since the index $i$ is last with the property $U_i \geq k$ we also know $\frac{|U|}{m ^ {i + 1}} < k$. 
