...
\end{displaymath}
For every choice of all $r_i$, $0 \leq i \leq n$, we obtain an unique solution of the above system of equations since we are in the field $\mathbb{Z}_N$. The number of all choices of $r_i$ is ${\left\lceil \frac{N}{m} \right\rceil}^{n + 1}$. From the number of hash functions which map $x_i$ to $y_i$ for $1 \leq i \leq n$ it follows
\begin{displaymath}
...
Hence this system is nearly strongly $n+1$-universal with the constant $\frac{{\left\lceil \frac{N}{m} \right\rceil}^{n + 1}}{(\frac{N}{m})^{n+1}}$.
\end{proof}
...
\end{theorem}
\begin{proof}
To prove the statement assume that $x_1, \dots, x_k$ are pairwise distinct elements of $U$ and $y_1, \dots, y_k$ are elements of $B$. For every $y_i$, $1 \leq i \leq k$ choose a pair $(a_i, b_i)$ with $o(a_i, b_i)=y_i$. The number of such choices is $m ^k$. From Theorem \ref{theorem-pair-system} for a fixed choice of $(a_i, b_i)$, $1 \leq i \leq k$, it follows that the probability of the event \[ h(x_1) = a_1, \dots, h(x_k) = a_k \text{ and } i(x_1) = b_1, \dots, i(x_k) = b_k \] is less than $\frac{1}{m^{2k}}$. Hence $\Prob{f(x_i} = y_i, 1 \le i \le k) = \frac{m^k}{m^{2k}} = \frac{1}{m^k}$. Now we see the system $F$ is strongly $k$-universal.
\end{proof}
...
Let $H$ be a strongly $k$-universal system of functions mapping universe $U$ to a table $B$ of size $m$. Size of the system is then bounded by:
\begin{displaymath}
...
Let $i$ be the greatest index with $|U_i| \geq k$. Functions $h_1, h_2,\dots, h_i$ map at least $k$ elements of set $U_i$ to a singleton. By Remark \ref{remark-n-to-k}, the probability of this event is less or equal to $\frac{1}{m ^ {k-1}}$. It follows that $i \leq \frac{|H|}{m ^ {k - 1}}$. Since index $i$ is the greatest index with the property $U_i \geq k$ we also have that $\frac{|U|}{m ^ {i + 1}} \leq |U_{i + 1}| < |U_i| \leq k$. 
