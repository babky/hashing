...
\begin{section}{Important facts from linear algebra}
...
\begin{proof}
Let $X_v = |A \cup (v + A)|$ be a random variable for a random uniformly chosen vector $u \in V$. We need to compute $\Expect{X_v}$ taken through all choices of $v \in V$.
\[
\Expect{X_v} = \displaystyle\sum_{v \in V} |A \cup (v + A)| . \Prob{v \text{ is chosen}} = \displaystyle\sum_{v \in V} \frac{|A \cup (v + A)|}{|V|}
\]

If we uniformly choose another vector $u \in V$ independent of $v$ then from standard definition of probability we obtain
\[
\Prob{u \in A~\vee u \in v + A} = \frac{|A \cup (v + A)|}{|V|} \text{.}
\]

Because $|A| = |v + A| = |V|(1 - \mu)$ probabilities of both events $u \notin A$ and $u \notin v + A$ are equal to $\mu$. These events are independent because vectors $u$ and $v$ are chosen independently of each other. So we can directly compute the above probability.
\[
\Prob{u \in A \vee u \in v + A} = 1 - \Prob{u \notin A \wedge\ u \notin v + A} = 1 - \mu^2
\]

From the above fact we express $||A \cup (v + A)|}{|V||$.
\begin{displaymath}
|A \cup (v + A)| = |V| \Prob{u \in A~\vee u \in v + A} = |V|(1 - \mu^2)
\end{displaymath}

Now we substitute the just obtained result into the computation of $\Expect(X_v)$.
\[
\Expect{X_v} = \displaystyle\sum_{v \in V} \frac{|A \cup (v + A)|}{|V|}
\]
...
\Expect{1 - \frac{|A \cup (v + A)|}{|V|}}
	...
	& = \frac{\sum\displaylimits_{v \in V}\mu ^ 2}{|V|}
\end{split}
\end{displaymath}
\end{proof}
\end{lemma}
