...
\section{Algorithms}
In Section \ref{section-proposed-model} we propose a model of universal hashing without any exact definition. Precise algorithms, showing how the operations work, are required. Despite the fact that the algorithms are very similar to those shown in Chapter \ref{chapter-hashing}, they are described more precisely again.

Although Theorem \ref{theorem-model-chain-limit-rule} gives the chain length limit for $\alpha \in \{1, 1.5\}$ and $p = 0.5$, different parametrisation is possible as well. Algorithm \ref{algorithm-scheme-3} from the proof of the theorem is general enough to be able to compute the bound for arbitrary $\alpha > 0$ and prescribed $p \in (0, 1)$. According to Theorem \ref{theorem-model-chain-limit-rule} the limit function is in the form $a \log \log m + b \log m$ for $a, b \in \mathbb{R}$ depending on $\alpha$ and $p$.

Let us describe the hash table's variables. It contains variables storing its size $Size$, the number of represented elements $Count$ and array $T$ is the hash table itself. Every bucket $T[i]$ has associated two variables $T[i].Size$ and $T[i].Chain$ -- its size and the linked list of the elements in the bucket. Function $Limit$ denotes the chain length limiting function and $Hash$ is the current universal function -- a linear transformation represented by a binary matrix.

Initialisation creates a new empty table with the prescribed size. Note that is also uniformly chooses a random linear mapping by the random initialisation of bits of the matrix. The values of bits are chosen with the same probability $0.5$.
...
Whenever a load factor rule or the chain limit rule is violated, the table is rehashed using a new function chosen in initialisation. Initialisation, Algorithm \ref{algorithm-initialisation}, allows a parameter $m$ specifying the new size, so the table is rehashed into a possibly larger or smaller table.

Find is very straightforward. Let us notice that the linked list manipulation operations return \textbf{true} in the successful case, if an element is found, deleted or inserted, and otherwise return \textbf{false}. Insert is slightly complicated compared to the original one. The checks for both rules are required and a rehash is done if needed.  The only difference to the classic delete is the load factor rule check.
...
In Section \ref{section-time-complexity} we estimate the expected amortised time complexity using the potential method. So let us explain it first. Assume that we have a data structure and a sequence of operations performed $\{o\}_{i = 1}^{n}$ on it. We want to estimate the running time of every operation. Of course, we can use the worst case time. Now consider a simple array when we put the elements to the end of the array. If there is a free position, we store the element. When the array is full, then we double the array and store the element. Question is, what time is needed to store $n$ elements in such array. The worst case analysis gives the result $O(n ^ 2)$. Amortised analysis gives a far better result, $O(n)$. This result can be seen in a way that the fast inserts accumulated some potential that is used by a following slow insert. The amortised time of the operation thus may be seen as the time consumed by all operations uniformly distributed across the operations. To bound the amortised complexity of every operation in general, we must take the supremum through all the sequences of the same length.
\begin{definition}
Let $\{o\}_{i = 1}^{n}$ be the sequence of $n \in \mathbb{N}$ operations. Let $T_o$ be the time required to perform the operations $o_i$ for $i = 1, \dots, n$ in this order. Then the amortised complexity of an operation is $\lim\limits_{n \rightarrow \infty}\frac{\sup_{o}T_o}{n}$.
\end{definition}

Now we show a potential method -- method how to estimate the amortised complexity. First, we define a potential function, $\{p\}_{i = 0}^{n}$. The constant $p_0$ is the starting potential, often chosen as zero. After each operation the data structure is associated with a new potential $p_i$. The operations not only change the data structure, they change its potential, too. Put $a_i = t_i + p_{i} - p_{i - 1}$ as the amortised complexity of the $i$\textsuperscript{th} operation where $t_i$ is its running time. The time taken by the operations, $T_o = \sum\displaystyle_{i = 1}^{n} t_i$, may then be estimated by the amortised time, $A_o = \sum\displaystyle_{i = 1}^{n} a_i$, as
\[
...
\[
	t = \lim\limits_{n \rightarrow \infty} \frac{\sup\displaylimits_o T_o}{n} = \lim\limits_{n \rightarrow \infty} \frac{\sup\displaylimits_o (A_o + p_0 - p^o)}{n} \text{.}
\]
...
\begin{enumerate}
\item [(1)] If $p_0 = 0$, then $T = A - p^o$. An operation's amortised complexity then equals \[ \lim\limits_{n \rightarrow \infty} \frac{\sup\displaylimits_o A_o}{n} - \lim\limits_{n \rightarrow \infty} \frac{\sup\displaylimits_o p^o}{n} \text{.} \] It is constant if for every sequence $\{o_i\}_{i = 1}^{n}$ $a_i$ is constant and $\lim\limits_{n \rightarrow \infty} \frac{\sup\displaylimits_o p^o}{n} < \infty$.
...
The fraction $\frac{(ce - r)m}{o}$ converges to zero with the growing number of operations $o$. This follows from the Weak Law of Large Numbers -- Theorem \ref{theorem-weak-law-of-large-numbers} and from the fact that $o \in \Omega(m)$. To be precise, define the random variable $C_i$ as the number of rehashes required to fix the chain limit rule in the $i$\textsuperscript{th} l-cycle for $i = 1, \dots, c$. The random variable $X_i = m C_i$ then denotes the time required to fix the chain limit rule in the $i$\textsuperscript{th} l-cycle. From Lemma \ref{lemma-linear-transformations-tries} we have that $\Variance{C_i} \leq c$ for a constant $c \in \mathbb{R}^{+}$. By using Lemma \ref{lemma-expected-value-properties} we obtain that $\Variance{X_i} = \Variance{mC_i} = m ^ 2 \Variance{C_i} = cm ^ 2$.

Clearly $\Expect{X_i} = em$, $r = \sum\displaystyle_{i = 1}^{c} C_i$, $o \geq m \geq c > 0$ and $\Variance{X_i}$ is finite. It follows that
\[
\begin{split}
\left|\frac{(ce - r)m}{o}\right|
	& \leq \left|\frac{cem - \sum\displaylimits_{i = 1}^{c} X_i}{c}\right| \\
	& \leq \left|\frac{c\Expect{X_1}}{c} - \frac{\sum\displaylimits_{i = 1}^{c} X_i}{c}\right| \\
	& = \left|\Expect{X_1} - \frac{\sum\displaylimits_{i = 1}^{c} X_i}{c}\right| \xrightarrow{c \rightarrow \infty} 0 \text{.}
\end{split}
\]

By Claim \ref{claim-amortised-complexity} we have that the amortised complexity of every operation is constant.
\end{proof}
