...
At last, define the \emph{third point} $x_2 = x_1 ^ 2 = 2 ^ {-2 ^ {-c - 1}}$. The inequality $x_2 > x_s$ follows from $-c - 1 < -c + 1 - \log e$. The point $x_2$ then lies in the decreasing phase of the function $\frac{f_0(x)}{x}$. For the point $x_2$, the next statement holds:
\[
\begin{split}
\frac{f_0(x_2)}{x_2} 
...
The second fact comes from the statement shown immediately after the definition of the point $x_2$.
...
\subparagraph{Let $x \in [x_1, \mu_0]$.} Because of the choice $x \geq x_1$ we have that $f_0(x) = 1$ and it follows that 
\[ 
...
The greatest disadvantage of the current estimate of the constant $c_\epsilon$ is its high inaccuracy. As shown later in Theorem \ref{theorem-n-logn-to-n} and Theorem \ref{theorem-n-to-n} this constant plays a crucial role for the multiplicative constant. For a practical use of the results of the theorems we need the smallest value possible. Next, we try to lower this value as much as possible. When we successfully manage the task, then we are able to propose a reasonable chain limit rule bounding the length of the longest chain.
\end{section}

As observed in the next corollary, we may apply Theorem \ref{theorem-set-onto-by-linear-transform} for two affine vector subspaces and an affine linear transform between them, too.
...
The bound on the investigated probability is obtained from Corollary \ref{corollary-f-decreasing} and by putting together all the above inequalities:
\[
\begin{split}
& \Prob{T_1(T_0(S)) \neq \vecspace{b} \wedge |T_0(S)| > \frac{|S|}{k}} \\
