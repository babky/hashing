...
\begin{definition}[Event $E_1$ from \cite{alonetal}]
...
\begin{definition}[Event $E_2$ from \cite{alonetal}]
...
From now on we reserve the following symbols for the decomposition; we assume that $u, f, b \in \mathbb{N}$, $f \geq b$, $T_0 \in_U \linearmaps{u}{f}$ and $T_1 \in_U \surjectivelinearmaps{f}{b}$ and $T = T_1 \circ T_0$.
Refer to \cref{fig-decomposition-general} for a better picture of the decomposition. 
\begin{figure}
	\caption{The decomposition of $T$. Let us note that $F_A = T_1^{-1}(\vec{y})$, $U_A = T^{-1}(y)$ and $S_A = S \cap U_A$.}
...
\end{center}
\end{figure}
Next we show $E_1$ is strongly related to event $E_2$.

\begin{proposition}[Proposition 3.2 from \cite{alonetal}]
...
\begin{proposition}[Proposition~3.1 of \cite{alonetal}]
...
From \cref{proposition-prob-bound} it follows that $\probs{T_1\in_U \surjectivelinearmaps{f}{b}}{T_1(\binvecspace{f} - T_0(S)) \neq \binvecspace{b}} \leq \alpha ^ {f - b - \log b + \log \log \alpha^{-1}}$ where $\alpha = 1 - \frac{|\binvecspace{f} - T_0(S)|}{|\binvecspace{f}|} = \frac{|T_0(S)|}{2^f} \leq \frac{|S|}{2^f} = \mu = 2^{b - f} < 1$.
...
\begin{proposition}[Proposition~3.4 in \cite{alonetal}]
